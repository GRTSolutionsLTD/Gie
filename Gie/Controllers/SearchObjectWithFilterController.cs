...
{
    public class SearchObjectWithFilterController : ApiController
    {
        /// <summary>
        ...
        public JObject Post(JObject input)
        {
            ...
            try
            {
                ...
                if (objectArray.Count() == 0)
                {
                    ...
                    //return JArray.Parse("[{message: 'No matching entries'}]");
                }
                else
                {
                    //int counter = 0;
                    ...
                    {
                        result.Append(JObject.Parse(objet.ToString())).Append(",");
                        //counter++;
                    }
                    result.Length--;
                    result.Append("],Length:'").Append(objectArray.Count()).Append("'}");
            ...
            catch (Exception e)
            {
                try
                {
                    ...
                    //return JArray.Parse("[{error: '" + e.Message.ToString() + "'}]");
                }
                catch
                {
                    return JObject.Parse("{error: 'Geting data from the json, failed'}");
                    //return JArray.Parse("[{error: 'Geting data from the json, failed'}]");
                }
            }
        }
    }
